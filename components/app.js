...
const app = new Vue({
  ...
  data : {
      ...
      // Took mounted attribute out of data attribute

      // I dont know what this is or why you are using it so i am taking it out
      // Also dont know why you are trying to use getItem and load data when it is already hard-coded

      /*mounted() {
        if (localStorage.getItem('addToWatchList')) {
          try {
            this.addToWatchList = JSON.parse(localStorage.getItem('addToWatchList'));
          } catch(e) {
            localStorage.removeItem('addToWatchList');
          }
        }
      },*/


      // took methods attribute out of data attribute
      methods: {
      ...
      }// removed uneccesary comment here
})
